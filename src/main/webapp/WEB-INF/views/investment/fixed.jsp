...
<body class="container">
	...
	<form action="fixed">
		...
		<div class="form-group">
			<label for="initialAmount">Initial Amount</label> <input
				type="number" min="0" step=".01" class="form-control"
				id="initialAmount" name="initialAmount" value='${initialAmount}'
				placeholder="0.00">
		</div>
		<div class="form-group">
			<label for="rate">Interest Rate(%)</label> <input type="number"
				min="0" step="any" class="form-control" id="rate" name="rate"
				value='${rate}' placeholder="0.00">
		</div>
		<div class="form-group">
			<label for="months">Investment Term</label> <input type="number"
				min="0" step="1" class="form-control" id="months" name="months"
		...
		<c:choose>
			<c:when test="${not empty param.initialAmount}">
				...
				<button type="submit" class="btn btn-primary"
					formaction="/finance-1.0/investment/save">Save</button>
			</c:when>
			<c:otherwise>
				<div class="form-group">
					<label for="name">Name: </label> <input type="text"
						class="form-control" id="name" name="name"
						placeholder="Forecast Name" disabled>
				</div>
